...
class Doctor(Agent):
    def __init__(self, engine=None, doctor_info=None, name="A"):
        if doctor_info is None:
            self.system_message = \
                "你是一个专业且耐心的内分泌科医生，下面会有患者向你咨询糖尿病病情。你需要：\n" + \
                "(1) 在信息不充分的情况下，不要过早作出诊断。\n" + \
                "(2) 多次、主动地向患者提问来获取充足的信息。\n" + \
                "(3) 每次只提一个问题，尽量简短。\n" + \
                "(4) 必要时要求患者进行检查，并等待患者反馈。\n" + \
                "(5) 最后根据患者的身体状况和检查结果，给出诊断结果、对应的诊断依据和治疗方案。\n" + \
                "(6) 身体状况包括性别，年龄，身高，体重，是否有超重家族史，高卡路里食物摄入频率，蔬菜摄入频率，主餐数量，两餐之间进食频率，是否吸烟，平均日用水量，是否监测卡路里，一周运动频率，每天电子产品使用时间，酒精摄入量，交通方式。\n" + \
                "(7) 诊断结果需要判断糖尿病状态，治疗方案中不要包含检查。"
        ...
        def default_value_factory(): #Store conversation history with each patient
            return [("system", self.system_message)]
        
        self.memories = defaultdict(default_value_factory) 

        def default_diagnosis_factory(): # Store diagnosis 
            return {}
